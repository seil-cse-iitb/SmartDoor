...
def Add_Entry_Exit_Details( SessionID, RoomID, Direction, Height, Weight ):
	(con, cursor) = ConnectDataBase()
	Time_Of_EE =  time.strftime("%H:%M:%S")
	Day_Of_EE =  time.strftime("%Y-%m-%d")
	EE_TimeDate  = Day_Of_EE + " " + Time_Of_EE
	values = "'" + str(SessionID) + "', '" + EE_TimeDate + "', '" + str(RoomID)+ "', '" + Direction + "', '" + str(Height)+ "', '" + str(Weight)+ "')"
	sql= """INSERT INTO `""" + DB +"""`.`SmartDoor_PeopleEntryExitDetail` 
		(`SID`, `TimeOfEntryExit`, `RoomID`, `Direction`, `Height`, `Weight`) 
		VALUES (""" + values
	try:
		cursor.execute(sql)
		con.commit()
...
def ConnectToServer(PORT):
	...
	socket.socket.reconnectionDelay = 0
	MsgDisplayFlag = True
	while NotConnectedToServer:
		try:
			...
			if MsgDisplayFlag:
				Failure_TimeStamp =  time.strftime("%Y-%m-%d %H:%M:%S")		
				FailureEvent = "Unable to connect or Disconnected from the Server"
				Local_Global = 1 #Local is 0, Global is 1
				FailedAttempts = '' 
				DiagnosticsReportError( Failure_TimeStamp, FailureEvent, Local_Global, FailedAttempts )
...
if __name__ == '__main__':
	...
	while True:
		...
		try:
			...
			if len(datachunks) == 2:
				datachunks[1]= datachunks[1].strip('\r\n')
				if datachunks[0] == 'R' and datachunks[1] == 'X': # "R/X"
					print("Signal to start recording through the camera")
					Session_ID =  Get_Latest_SID() 
					session_id_str = str(Session_ID) + "/R" #check if it is proper session ID
					ServerHandle = ConnectToServer(CAMPORT)
					ServerHandle.send(session_id_str)
					ServerHandle.close()
			elif len(datachunks) == 3:
				...
				Session_ID =  Get_Latest_SID() 
				Add_Entry_Exit_Details( Session_ID, RoomID, Direction, Height, Weight )	
				UpdatePeoplePresent(Session_ID, RoomID, NoOfPeople)
				session_id_str = str(Session_ID) + "/E/" + datachunks[0]
				ServerHandle = ConnectToServer(INFOPORT)
				ServerHandle.send(session_id_str)
				ServerHandle.close()
		except serial.serialutil.SerialException:
			ArduinoSer = ConnectArduinoSerialPort()
